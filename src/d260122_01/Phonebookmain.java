			...
			if(scan.hasNextLine()) {
			 ...
			 if(pm.isExist(searchname)) {
				 ...
				 */
				 ...
				 //변경사항이 없을 경우 기존의 데이터를 가지고 와서 입력처리
				 d260122.Phonebook p=pm.getPhonebookByName(searchname);
				 //값이 빈란이 아닐 때 phonebook객체를 변경해준다.
				 if(!name.equals("")) { p.setName(name); }
				 if(!hp.equals("")) { p.setHp(hp); }
				 if(!email.equals("")) { p.setEmail(email);}
				 //System.out.println(p);
