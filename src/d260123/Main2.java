...
public class Main2 {
	...
	public static void main(String[] args) throws IOException {
		...
		//스트림에 아무것도 들어 이지 않을 때는 -1
		//in.read()함수는 스트림에 값이 없을 때는 키보드 입력대기 생태
		...
		*/
		...
		//위의 코드의 문제점:출력용으로 화면에만 글자가 나타나고 실제 사용할 경우
		//해당 데이터를 다시는 이용할 수 없는 문제 발생
		//이를 해결하기 위해 문자열을 누적시켜야함.
		//아래의 코드는 String 변수로 인해 발생하는 문제(EOF-End Of File)
		/*
		String str="";
		while((key=System.in.read())!=1) {
			str+=(char)key;
		...
		//str+=(char)key;
		System.out.println("전체문자열 출력:"+str);
		...
		*/
		...
		System.out.println(bytes); //객체만 출려되는 문제
		//byte배열을 문자열로 변환
